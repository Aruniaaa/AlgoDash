...
def get_codeforces_problems(tags=None, min_rating=None, max_rating=None, limit=50):
    ...
    try:
        for tag in tag_batches:
            ...
            stats_map = {
                f"{s.get('contestId')}_{s.get('index')}": s
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if upcoming:
                ...
                filtered_contests = [c for c in contests if c.get('phase') == 'FINISHED']
            ...
            for contest in filtered_contests[:10]:  
                ...
                standardized_contests.append({
                    ...
                    'is_contest': True,
                    'contest_start': datetime.fromtimestamp(start_time).isoformat() if start_time else None,
                    'contest_end': datetime.fromtimestamp(start_time + duration_seconds).isoformat() if start_time else None,
...
def get_most_used_lang(handle):
    ...
    if res.status_code == 200:
        ...
        lang_count = {}
        for result in data:
            lang_count[result['programmingLanguage']] = lang_count.get(result['programmingLanguage'], 0) + 1
            ...
            if difficulty_level in tag_data:
                for tag_info in tag_data[difficulty_level]:
                    ...
                    if problems_solved > 0:
                        ...
                        standardized_tag = tag_mapping.get(leetcode_tag, leetcode_tag)
                        
      
                        if standardized_tag in tags_list_all:
                            tag_counts[standardized_tag] = tag_counts.get(standardized_tag, 0) + problems_solved
...
def get_unified_tag_distribution(leetcode_username=None, codeforces_handle=None):
    ...
    print(f"Inside the unified tag distribution func | Leetcode: {leetcode_username} | Codeforces: {codeforces_handle}")
            ...
            if include_contests:
                ...
                for contest in lc_contests_raw:
                    all_contests.append({
                        ...
                        'is_contest': True,
                        'contest_start': datetime.fromtimestamp(contest.get('startTime', 0)).isoformat(),
                        'contest_end': datetime.fromtimestamp(contest.get('startTime', 0) + contest.get('duration', 0)).isoformat(),
